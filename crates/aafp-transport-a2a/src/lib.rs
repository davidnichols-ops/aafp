...
impl AafpA2aTransport {
    ...
    pub async fn connect_with_auth(
        ...
        let auth_ctx = auth_provider
            ...
            .await
            .map_err(|e| AafpA2aError::Session(format!("authorization denied: {e}")))?;
        session
            .on_authorization_verified(auth_ctx)
            .map_err(|e| AafpA2aError::Session(e.to_string()))?;
        ...
        // 6. Open bidirectional stream for A2A messages
...
fn extract_tls_binding(conn: &QuicConnection) -> Result<[u8; 32], AafpA2aError> {
    conn.export_tls_binding(TLS_EXPORTER_LABEL.as_bytes(), &[])
        .map_err(|e| AafpA2aError::Sdk(SdkError::Handshake(e.to_string())))
}
